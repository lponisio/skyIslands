...
ppint.dis <- function(spec.dat, types= c("GenSp", "PlantGenSp", "Int"),
  ...
  f <- function(){
    layout(1)
    plot(c.pol~c.dist, pch =16, col="black", ylim=c(0,1), xaxt="n",
         xlab="distance", ylab="dissimilarity", las=1, cex=1.2)
    points(c.plant~c.dist, pch=16, col="darkolivegreen", cex=1.2)
    points(c.int~c.dist, pch=16, col="darkred", cex=1.2)
