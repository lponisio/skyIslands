...
# Create a plot
bray_curtis_plot <- ggplot(data = pcoa_df, aes(x=pcoa1, y=pcoa2, color=species_data$gensp, shape=species_data$Genus)) +
  geom_point(aes(size=1)) +
  stat_ellipse(geom = "polygon",
               linetype= species_data$Genus) %>%
  labs(x = "PC1",
       y = "PC2", 
       title = "Microbe PCoA") +
  theme(title = element_text(size = 10)) + # makes titles smaller
  theme_classic() + #labs(color='Bee Genus') + theme_classic() +
          scale_color_manual(name='Species', values=c('#6baed6','#3182bd','#08519c', #3 anothophora sp -- blue
