...
phylotree_heatmap_byGenus <- function(tree.object, metadata, this.genus, presAbsTable, site.order, all_levels=TRUE, levels_to_drop, clade_names=NULL, do_collapse=FALSE, tree.type){
...
#browser()
  ...
  p2 <- p +
    ...
    ggtitle(paste(tree.type, this.genus, this_level)) 
  p2
  } else if (tree.type=='rbcl'){
    #read in the taxonomic info for each feature
    feature.2.tax.rbcl <-
    ...
    ## match the tip labs to the table with feature ID and Taxon
    gentree$tip.label  <-  feature.2.tax.rbcl$Taxon[
      match(gentree$tip.label,
            feature.2.tax.rbcl$FeatureID)]
    ...
    #     }
    #     #make a list of tip labels that include the specified level
    #     tips_to_modify <- grep(drop.level, gentree$tip.label)
    #     if (length(tips_to_modify) > 0) {
    #       # Replace the pattern and everything following it with an empty string
    #       gentree$tip.label[tips_to_modify] <- gsub(drop.level, "", gentree$tip.label[tips_to_modify])
    #       gentree$tip.label[tips_to_modify] <- gsub("_sp.", "", gentree$tip.label[tips_to_modify])
    #       gentree$tip.label[tips_to_modify] <- gsub("_$", "", gentree$tip.label[tips_to_modify])
    #       this.tip.level.label <- paste(level)
    #       new_len <- length(gentree$tip.label)
    #       percent_drop <- (length(tips_to_modify) / orig_len) * 100
    #       print(paste(level, "percent tips dropped", percent_drop))
    ...
    features_site_metadata <- match_shared_ID(matched_pres_meta, meta_match_sites) %>%
      right_join(meta_match_sites, by='UniqueID') %>%
      pivot_longer(cols = starts_with('RBCL'), names_to = 'pollen', values_to = 'pollen_pres') %>%
      group_by(pollen) %>%
      filter(pollen_pres == 1) %>%
      select(!pollen_pres) %>%
      relocate(pollen) %>%
      group_by(pollen) %>%
      add_count(Site, name="n_individuals") %>%
    ...
    #dropping branches that weren't in the presence abs table
    final_drop <- gentree$tip.label[!(gentree$tip.label %in% features_site_metadata$pollen)]
    #browser()
...
if(tree.type=='16s'){
  ...
  meta <- spec16s %>%
    select(all_of(meta_cols), Apidae, starts_with('RBCL')) %>%
    na.omit() %>%
    select(!starts_with('RBCL')) %>%
    filter(Apidae == 1) #%>%
  comm_presabs <- as.data.frame(indiv.comm.rbclR0) #load in the pres/abs table
  comm_presabs[comm_presabs > 0] <- 1 #change all rel abund to 1
  comm_presabs <- tibble::rownames_to_column(comm_presabs, "UniqueID") #ma
...
# apis_tree
# 
apis_tree2 <- phylotree_heatmap_byGenus(physeq16sR0, meta, "Apis", comm_presabs, apis_sites, all_levels=TRUE, do_collapse = TRUE, tree.type='16s')
apis_tree2
# 
...
apis_tree_rbcl <- phylotree_heatmap_byGenus(tree.object=physeqRBCLR0,
                                            metadata=meta,
                                            this.genus="Apis",
                                            presAbsTable=comm_presabs,
                                            site.order=apis_sites,
                                            all_levels=TRUE,
                                            #levels_to_drop=c('species', 'genus'),
                                            do_collapse = TRUE,
                                            tree.type='rbcl')
apis_tree_rbcl
