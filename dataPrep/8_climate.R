...
sample_windows <- weather %>%
  ...
  summarize(
    start_date = min(StartDate, na.rm = TRUE),
  ...
  mutate(
    ...
    # Define cumulative window start (May 1 for first round, previous end for later rounds)
    window_start = lag(end_date, default = first(baseline_start)),
    window_end   = start_date - days(1)
  ) %>%
  ungroup()
...
precip_combined <- round_precip %>%
  ...
  mutate(
    ...
    Cumulative_Precip = Spring_Precip + cumsum(lag(Round_Precip, default = 0))
  ) %>%
  ungroup() %>%
...
#   group_by(Site, Year) %>%
#   mutate(
#     prev_end = lag(end_date, default = first(baseline_start)),  # baseline_start of first round in the group
#     window_start = prev_end,
#     window_end   = start_date - days(1)  # cumulative up to day before the round starts
#   ) %>%
#   ungroup()
# 
...
#       Date <= rw$window_end
#     ) %>%
#     summarize(Cumulative_Precip = sum(Precip, na.rm = TRUE))
#   
...
#     Cumulative_Precip = site_precip$Cumulative_Precip
#   )
# }
# 
...
#           daily_precip_data$Date >= start_date &
#           daily_precip_data$Date <= end_date
#       ],
#       na.rm = TRUE
#     )
#   )
...
#   mutate(
#     summer_start = as.Date(paste0(Year, "-05-01")),
#     end_month = floor_date(start_date - days(1), "month")
#   ) %>%
